...
export default async (config) => {
  ...
  const { issueType, ticketPrefix = config.jiraTicketPrefix } = await prompts([
    { type: 'select', name: 'issueType', message: 'Issue Type', choices:
    config.statuses.map((taskType) => ({
      title: taskType.name, value: taskType.name,
    })) },
    ...config.projectChoices?.length ? [
      { type: 'select', name: 'ticketPrefix', message: 'Ticket Prefix', choices: config.projectChoices.map((choice) => ({
  ...
  if (await confirm('Assign to yourself?')) {
    data.fields.assignee = { accountId: accountId };
  }

  const ticket = await jiraApiClient.createTicket(data);

  ticket.summary = prefix + projectPrefix + summary;
