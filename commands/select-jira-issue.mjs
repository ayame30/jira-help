...
export default async (config) => {
  ...
  const ticketChoices = [
    ...
    ..._.orderBy(issues, ['sprintName', 'status'], ['asc', 'asc']).map((issue) => ({
      title: getTicketTitle(issue),
      value: issue,
    })),
  ];

  const { issue } = await prompts([{ type: 'select', name: 'issue', message: `Pick A Ticket`, choices: ticketChoices }], { onCancel: () => process.exit(0) });
