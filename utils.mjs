...
function displayConfig(config) {
  if (config) {
    ...
    console.log(chalk.green('API Token:'), config.apiToken ? '********' : 'Not set');
...
async function confirm(message) {
  const { confirm } = await prompts([{ type: 'confirm', name: 'confirm', message, initial: true }], {
    onCancel: () => {
      process.exit(0);
    },
  });
...
async function promptText(message, initial) {
  const { answer } = await prompts([{ type: 'text', name: 'answer', message, initial }], {
    onCancel: () => {
      process.exit(0);
    },
  });
...
function getColoredSprint(sprintName) {
  return sprintName ? chalk.bgGreen(sprintName.padEnd(sprintLengthMin, ' ')) : chalk.bgBlue('Backlog'.padEnd(sprintLengthMin, ' '));
}
